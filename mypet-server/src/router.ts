...
router.get('/posts', async (req, res, next) => {
  const allRecords = await model.fetch(postCollection);
  res.status(200).json({
      data: allRecords,
      msg: `Fetch success!`
  });
...
router.get('/posts/:id', async (req, res, next) => {
  ...
  try {
      const record = await model.get(postCollection, id);
      res.status(200).json({
          data: record,
          msg: `Get success!`
      });
...
// eg:  /users/filter/{"_id":"5ea3dfa5ca10e82157843225"}
// eg:  /users/filter/{"firstname":"bingfei","lastname":"ma"}

router.get('/posts/filter/:filter', async (req, res, next) => {
  try {
    const query = JSON.parse(req.params.filter as string);
    if (query._id) {
      query._id = new ObjectID(query._id);
    }
    const records = await model.fetch(postCollection, query);
    res.status(200).json({
        data: records,
        msg: `Query success!`
    });
...
router.post('/posts', async (req, res, next) => {
  const putData = req.body;
  try {
      const record = await model.put(postCollection, putData);
      res.status(200).json({
          data: record.ops[0],
          msg: `Put success ${putData.firstname}`
      });
...
router.patch('/posts', async (req, res, next) => {
  const updateDate = req.body;
  try {
      if (updateDate._id) {
          const record = await model.patch(postCollection, updateDate);
          res.sendStatus(200).json({
              data: record,
              msg: `Edit success ${record._id}`
          });
...
router.delete('/posts/:id', async (req, res, next) => {
  const delId = req?.params?.id;
  try {
      await model.del(postCollection, delId);
      res.sendStatus(200).json({
          msg: `Delete success!`
      });
  } catch(err) {
